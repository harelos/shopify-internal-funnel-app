...
window.NovaHairPopupConfig = {
  ...
  /* Measurement is deliberately separate from trigger tuning. A holdout of
   * zero records signal/eligibility data without changing shopper exposure. */
  measurement: {
    signalTracking: true,
...
(function loadNovaHairPopupControl() {
  ...
  function applyRemote(remote) {
    ...
    if (remote.measurement) {
      if (typeof remote.measurement.signalTracking === 'boolean') {
        cfg.measurement.signalTracking = remote.measurement.signalTracking;
      }
      if (/^[a-z0-9_-]{3,80}$/i.test(remote.measurement.experimentId || '')) {
        cfg.measurement.experimentId = remote.measurement.experimentId;
      }
      copyNumber(cfg.measurement, 'holdoutPercent', remote.measurement);
    }
    ...
    function request(attempt) {
      window.fetch(endpoint, {
        ...
        if (attempt < maxAttempts) {
          var retryDelay = attempt === 1 ? 1200 : 2800;
          window.setTimeout(function () { request(attempt + 1); }, retryDelay);
        }
        else finish(null);
      });
    }
