...
await check('Public GitHub-backed preview and actual B asset are reachable',async()=>{
 const target=process.env.GALLERY_PREVIEW_URL;if(!target)throw Error('Public preview URL missing');
 const {p,c}=await newPage();const response=await p.goto(target+'?variant=variant-b&view=page',{timeout:60000,waitUntil:'domcontentloaded'});
 if(!response||response.status()!==200)throw Error('HTTP '+response?.status());
 await p.waitForFunction(()=>window.NHGalleryLab?.state.firstImageReady,{},{timeout:30000});if(p.errors.length)throw Error(p.errors.join('\n'));
 const dimensions=await p.locator('#labGalleryB').evaluate(el=>({width:el.naturalWidth,height:el.naturalHeight}));await p.screenshot({path:out+'/public-preview-B-real.png',fullPage:true});
 await p.reload();if((await p.evaluate(()=>NHGalleryLab.state.decision.variant.key))!=='variant-b')throw Error('Forced public B changed');await c.close();return {url:target,httpStatus:response.status(),dimensions};
});
await browser.close();await fs.writeFile(out+'/results.json',JSON.stringify({scope:'Chromium HTTP navigation with real copied B assets and original public Shopify CDN A assets. No real Shopify app, checkout or WebView integration.',results},null,2));
