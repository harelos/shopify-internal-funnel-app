 ...
 *
 * Privacy: the hair photo is analysed on-canvas in the browser and never
 * uploaded, which is exactly what the Hebrew copy promises.
 */
(function () {
  'use strict';
  ...
  function analyticsBody(event, payload) {
    var key = event + ':' + popupVersion() + ':' + sessionId() + ':' + Date.now() + ':' + stepIndex;
    return {
      event: event,
      visitorId: visitorId(),
  ...
  function renderCard(spec) {
    ...
    if (spec.kind === 'shades') {
      ...
      F.shades.forEach(function (s) {
        ...
        var chip = el('span');
        /* Real cropped hair photo, with the sampled hex as the fallback so
         * the card still reads correctly if an asset fails to load. */
        chip.style.background = s.hex;
        if (s.img) {
          chip.style.backgroundImage = 'url(' + asset(s.img) + ')';
      ...
      card.appendChild(br);
    } else if (spec.kind === 'roots') {
      card.appendChild(el('h4', null, 'כיסוי שורשים'));
      card.appendChild(el('div', 'nhai__sub', 'בנוי לתקופה שבין הצבע המלא לתור הבא'));
  ...
  /* The page's "pick the lighter of two" rule applies to adjacent natural
   * shades, not to an arbitrary near-tie with eggplant or wine red. */
  var SHADE_TIE_MARGIN = 18;
  var NATURAL_SHADE_RANK = { black: 0, dark_brown: 1, light_brown: 2 };
  ...
  function nearestShade(sample) {
    ...
    var best = scored[0];
    if (FACTS.tieBreakLighter && scored[1] && (scored[1].d - best.d) <= SHADE_TIE_MARGIN) {
      var bestRank = NATURAL_SHADE_RANK[best.shade.key];
      var secondRank = NATURAL_SHADE_RANK[scored[1].shade.key];
      if (typeof bestRank === 'number' && typeof secondRank === 'number' && secondRank > bestRank) {
        best = scored[1];
      }
    }
    return {
  ...
  function renderPhotoStep(node) {
    ...
    input.addEventListener('change', function () {
      ...
      img.onload = function () {
        ...
        setTimeout(function () {
          ...
          var match = nearestShade(sample);
          URL.revokeObjectURL(url);
          sayUser('שלחתי תמונה');
          if (!match || !match.reliable) {
            logStep(node, 'photo_uncertain', { choiceId: 'manual_check' });
            say('לא הצלחתי לזהות את אזור השורשים בוודאות מהתמונה הזאת. בואי נבחר יחד.').then(function () {
              go(node.skipNext);
            });
            return;
          }
          context.shadeKey = match.shade.key;
          context.shadeLabel = match.shade.label;
          logStep(node, 'photo', { choiceId: match.shade.key, choiceLabel: match.shade.label });
          go(node.next);
        }, 1100);
      };
    ...
    footEl.appendChild(el('div', 'nhai__note', 'התמונה נבדקת במכשיר שלך ולא נשלחת. התוצאה היא כיוון ראשוני בלבד.'));
  }
  ...
  function renderOptions(node) {
    ...
    if (node.allowFree) {
      ...
      var submit = function () {
        ...
        /* A node can capture the answer instead of routing it (the name). */
        /* Learn her writing style from prose only. A one-word name is not a
         * writing sample, and counting it drags every profile to "terse". */
        if (window.NovaHairAITone && node.captureAs !== 'name') {
          window.NovaHairAITone.observe(text);
        }

        if (node.captureAs === 'name') {
          context.name = text.replace(/[^֐-׿a-zA-Z\s'-]/g, '').slice(0, 24).trim();
          go(afterName(node));
          return;
        }
  ...
  function renderCapture(node) {
    ...
    /* Phone is genuinely optional: no required flag, no nagging. */
    var phone = null;
    if (node.phoneOptional) {
      var pf = el('div', 'nhai__free');
      phone = document.createElement('input');
      phone.type = 'tel'; phone.placeholder = node.phoneLabel || 'טלפון (לא חובה)'; phone.autocomplete = 'tel';
      pf.appendChild(phone);
      form.appendChild(pf);
    }

    var consentWrap = el('label', 'nhai__note');
    var consent = document.createElement('input');
    consent.type = 'checkbox'; consent.required = true;
    consentWrap.appendChild(consent);
    consentWrap.appendChild(document.createTextNode(' ' + node.consentLabel));
    ...
    form.addEventListener('submit', function (e) {
      ...
      logStep(node, 'submit', {
        choiceId: 'captured',
        choiceLabel: phone && phone.value.trim() ? 'email+phone' : 'email'
      });
      emit('popup_submit_attempt', { attemptNumber: 1, consent: true, trigger: 'ai_concierge' });
      ...
      var capture = typeof OPTS.onCapture === 'function'
        ? Promise.resolve(OPTS.onCapture(email, phoneVal, context))
        : submitLead(email, phoneVal);
  ...
  function verificationTag() {
    ...
    return 'nhp_' + Array.prototype.map.call(bytes, function (byte) {
      return byte.toString(16).padStart(2, '0');
    }).join('');
  }

  function confirmLead(tag, signal) {
    var endpoint = OPTS.leadConfirmEndpoint || '/apps/funnels/api/popup/confirm-lead';
    var body = analyticsBody('popup_submit_success', { trigger: 'ai_concierge', consent: true });
    body.verificationTag = tag;
    return fetch(endpoint, {
      ...
      return response.json().catch(function () { return {}; }).then(function (data) {
        if (!response.ok || !data.confirmed) throw new Error(data.failureCategory || 'lead_not_confirmed');
        return data;
      });
    });
  }

  /* Submit through Shopify, then ask the server to verify the one-time tag and
   * marketing consent before recording a lead or attributing it to an order. */
  function submitLead(email, phoneVal) {
    var tag = verificationTag();
    var fd = new FormData();
    fd.append('form_type', 'customer');
    fd.append('utf8', '✓');
    fd.append('contact[email]', email);
    fd.append('contact[tags]', ['novahair-ai-concierge', 'novahair-exit-popup', 'exit-popup-lead', tag].join(','));
    fd.append('contact[accepts_marketing]', 'true');
    if (phoneVal) fd.append('contact[note]', 'phone: ' + phoneVal);
    ...
    return fetch('/contact', {
      ...
      if (!response.ok && response.type !== 'opaqueredirect' && response.status !== 0) {
        throw new Error('shopify_customer_form_' + response.status);
      }
      return confirmLead(tag, controller.signal);
    }).then(function (result) {
      if (window.NovaHairAttribution) {
        window.NovaHairAttribution.writeCartAttributes({
          conversationId: convId, agent: context.agent, lead: true
        });
        window.NovaHairAttribution.ga('lead', { agent: context.agent, hasPhone: Boolean(phoneVal) });
      }
      return result;
  ...
  function navigate(url, reason) {
    ...
    if (A) {
      ...
      if (/novahair-sales/.test(url) || reason === 'coupon') {
        A.writeCartAttributes({
          conversationId: convId, agent: context.agent, trigger: reason,
  ...
  function finish(how) {
    ...
    emit('popup_closed', { closeMethod: how === 'escape' ? 'esc' : 'other', trigger: 'ai_concierge' });
  ...
  /* After she gives (or skips) her name, a returning customer who has not
   * ordered in a while gets the warmer welcome-back instead of the cold open. */
  function afterName(node) {
    if (context.isReturning && (context.daysSinceLastOrder == null || context.daysSinceLastOrder >= 30)) {
      return 'welcome_back';
    }
    return node.next;
  }
  ...
  function routeAgent() {
    ...
    return {
      laneId: laneId,
      entry: lane.entry || F.entry,
      opener: lane.opener || null,
      /* Only the sales lane uses the generic angle opener + hook. The other
       * lanes' entry nodes greet her themselves, so a second opener would
       * double up. */
      sales: laneId === 'sales'
    };
  }
