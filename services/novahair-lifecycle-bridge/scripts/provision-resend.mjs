...
if (manifest.count !== 44 || automationFile.count !== 6) throw new Error("provisioning_manifest_incomplete");
...
await writeFile(
  ...
  `${JSON.stringify({ completed_at: new Date().toISOString(), templates: 44, events: eventDefinitions.length, automations: 6, all_automations_disabled: true, worker_registered: registered }, null, 2)}\n`,
  "utf8",
);
console.log(`Provisioned ${eventDefinitions.length} events, 44 published templates, and 6 disabled automations. Worker registration: ${registered ? "complete" : "not configured"}.`);
