...
test("master plan produces exactly 44 Hebrew-first lifecycle templates", () => {
  assert.equal(source.flows.length, 6);
  assert.equal(manifest.count, 44);
  assert.deepEqual(source.flows.map((flow: { emails: unknown[] }) => flow.emails.length), [10, 10, 5, 3, 12, 4]);
});
