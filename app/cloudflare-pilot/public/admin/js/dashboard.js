document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnDuplicateFunnel = document.getElementById("btn-duplicate-funnel");
  const modalDuplicateFunnel = document.getElementById("modal-duplicate-funnel");
  const formDuplicateFunnel = document.getElementById("form-duplicate-funnel");
  const btnCancelDuplicateModal = document.getElementById("btn-cancel-duplicate-modal");
  ...
  btnDuplicateFunnel?.addEventListener("click", () => {
    modalDuplicateFunnel.style.display = "flex";
  });
  btnCancelDuplicateModal?.addEventListener("click", () => {
    modalDuplicateFunnel.style.display = "none";
  });
      ...
      return `
        <div class="panel" style="cursor:pointer;" onclick="window.location.href='funnel.html?id=${f.id}'">
          ...
          <div class="panel-body">
            ...
            ${f.publicUrl ? `<div style="margin-top:12px; padding:10px; border:1px solid var(--line); border-radius:8px; background:#f8fff9;" onclick="event.stopPropagation();"><div class="eyebrow" style="margin-bottom:4px;">Public link</div><a href="${escapeHtml(f.publicUrl)}" target="_blank" rel="noreferrer" style="display:block; word-break:break-all; font-size:12px; margin-bottom:8px;">${escapeHtml(f.publicUrl)}</a><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation(); copyPublicLink('${escapeHtml(f.publicUrl)}')">Copy link</button></div>` : `<p class="muted" style="font-size:12px; margin-top:12px;">Draft only. Publish the funnel to get a public link.</p>`}
  ...
  window.copyPublicLink = async function(url) {
    try {
  ...
  formDuplicateFunnel?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const data = new FormData(formDuplicateFunnel);
    const submit = formDuplicateFunnel.querySelector('button[type="submit"]');
    if (submit) submit.disabled = true;
    try {
      const created = await API.post("/api/funnels/duplicate", {
        name: data.get("name"),
        slug: data.get("slug"),
        steps: [
          { name: "Listicle", kind: "ADVERTORIAL", sourceUrl: data.get("listicleUrl") },
          { name: "Sales Page", kind: "SALES", sourceUrl: data.get("salesUrl") },
          { name: "Checkout", kind: "CHECKOUT" },
        ],
      });
      modalDuplicateFunnel.style.display = "none";
      window.location.href = `funnel.html?id=${created.funnel.id}`;
    } catch (err) {
      alert("Failed to duplicate funnel: " + err.message);
    } finally {
