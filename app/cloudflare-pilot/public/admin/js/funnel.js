document.addEventListener("DOMContentLoaded", () => {
  ...
  const publicFunnelLink = document.getElementById("public-funnel-link");
  const publicFunnelUrl = document.getElementById("public-funnel-url");
  const btnCopyPublicFunnel = document.getElementById("btn-copy-public-funnel");
  ...
  btnCopyPublicFunnel?.addEventListener("click", async () => {
    const url = publicFunnelUrl?.getAttribute("href") || "";
    if (!url || url === "#") return;
    try {
  ...
  formAddVariant?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (sourceUrl) {
        await API.post(`/api/steps/${selectedStepId}/variants/import`, {
          name: data.get("name"),
          sourceUrl,
        });
      } else {
        await API.post(`/api/steps/${selectedStepId}/variants`, payload);
      }
  ...
  function renderHeader() {
    ...
    if (isPublished && currentFunnel.publicUrl && publicFunnelLink && publicFunnelUrl) {
      publicFunnelLink.style.display = "block";
      publicFunnelUrl.href = currentFunnel.publicUrl;
      publicFunnelUrl.textContent = currentFunnel.publicUrl;
    } else if (publicFunnelLink) {
      publicFunnelLink.style.display = "none";
    }
  }
        ...
        <div class="panel">
          <div class="panel-header">
            ...
            <div style="display:flex; gap:6px; align-items:center; flex-wrap:wrap; justify-content:flex-end;">
              ${v.sourceType === 'SHOPIFY_IMPORTED' ? '<span class="badge" title="Imported from a Shopify page">SHOPIFY IMPORT</span>' : ''}
          ...
          <div class="panel-body">
            ...
            ${v.sourceType === 'SHOPIFY_IMPORTED' && v.sourceUrl ? `<p class="muted" style="word-break:break-word; margin-bottom:12px;">Imported from <a href="${escapeHtml(v.sourceUrl)}" target="_blank" rel="noreferrer">${escapeHtml(v.sourceTitle || v.sourceUrl)}</a></p>` : ''}
