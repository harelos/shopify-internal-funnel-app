...
const POPUP_EVENTS = [
  ...
  "popup_email_submitted",
  "popup_coupon_revealed",
  "popup_dismissed",
  "popup_purchase",
] as const;

async function servePopupAnalytics(req: any, res: any) {
  try {
    const mode = analyticsModeForRequest(req.query as Record<string, unknown>);
    const { from, to } = req.query;
    const occurredAt: any = {};
    if (from) occurredAt.gte = new Date(from as string);
    if (to) occurredAt.lte = new Date(to as string);
    const events = await prisma.event.findMany({
      where: {
        isTest: isTestForMode(mode),
        name: { in: [...POPUP_EVENTS] },
        ...(from || to ? { occurredAt } : {}),
      },
    });
    const count = (name: string) => events.filter(event => event.name === name).length;
    const visitorCount = (name: string) => new Set(events.filter(event => event.name === name && event.visitorId).map(event => event.visitorId)).size;
    const emailVisitors = new Set(events.filter(event => event.name === "popup_email_submitted" && event.visitorId).map(event => event.visitorId));
    const purchaseVisitors = new Set(events.filter(event => event.name === "popup_purchase" && event.visitorId).map(event => event.visitorId));
    const attributedPurchases = count("popup_purchase");
    const matchedPurchases = [...purchaseVisitors].filter(visitorId => emailVisitors.has(visitorId)).length;
    return res.json({
      ...analyticsDataContract(mode),
      impressions: count("popup_impression"),
      formStarts: count("popup_form_started"),
      formSubmissions: count("popup_form_submitted"),
      emailSubmissions: count("popup_email_submitted"),
      couponReveals: count("popup_coupon_revealed"),
      dismissals: count("popup_dismissed"),
      attributedPurchases,
      emailToPurchaseRate: emailVisitors.size > 0 ? Number(((matchedPurchases / emailVisitors.size) * 100).toFixed(1)) : 0,
      popupVisitorCount: visitorCount("popup_impression"),
      note: "Purchases are counted only from Shopify orders/paid carrying the popup attribution line-item properties.",
    });
  } catch (err: any) {
    return res.status(500).json({ error: err.message || "Failed to generate popup analytics" });
  }
}
...
router.get("/analytics/:funnelId", async (req, res) => {
  try {
    ...
    if (funnelId === "popup") return servePopupAnalytics(req, res);
    if (funnelId === "account") {
...
// GET /api/analytics/popup is handled by the explicit guard in /analytics/:funnelId above.
...
router.post("/track", async (req, res) => {
  try {
    ...
    const allowedEvents = new Set(["page_view", "cta_click", "checkout_started", "FUNNEL_PAGE_VIEWED", "FUNNEL_CTA_CLICKED", "CHECKOUT_STARTED", ...POPUP_EVENTS]);
    const popupEvent = typeof event === "string" && event.startsWith("popup_");
    if (!event || !allowedEvents.has(event) || (!funnelId && !popupEvent)) {
      return res.status(400).json({ error: popupEvent ? "popup event is invalid" : "event name and funnelId are required" });
    }
    ...
    const funnel = funnelId
      ? await prisma.funnel.findUnique({ where: { id: funnelId }, include: { steps: { include: { variants: true } } } })
      : null;
    if (!popupEvent && !funnel) return res.status(404).json({ error: "Funnel not found" });
    const step = stepId && funnel ? funnel.steps.find(item => item.id === stepId) : undefined;
