...
import { fetchAndPrepareShopifyPage, ShopifyPageImportError } from "../lib/shopify-page-import.js";
...
// POST /api/steps/:stepId/variants/import — Fetch a public Shopify page and create a normal app variant
router.post("/steps/:stepId/variants/import", async (req, res) => {
  try {
    const { stepId } = req.params;
    ...
    if (typeof sourceUrl !== "string" || !sourceUrl.trim()) {
      return res.status(400).json({ error: "Shopify page URL is required" });
    }

    const step = await prisma.step.findUnique({ where: { id: stepId } });
    if (!step) return res.status(404).json({ error: "Step not found" });
    if (step.kind === "CHECKOUT") return res.status(400).json({ error: "Checkout step cannot have variants" });

    const config = getShopifyConfig();
    const imported = await fetchAndPrepareShopifyPage(sourceUrl, {
    ...
    const version = await prisma.contentVersion.create({
      data: {
        ...
        normalizedHtml: imported.normalizedHtml,
    ...
    res.status(201).json({
      variant: { ...variant, versions: [version] },
      imported: {
        sourceUrl: imported.finalUrl,
    ...
    if (err instanceof ShopifyPageImportError) {
      return res.status(err.status).json({ error: err.message });
    }
    res.status(500).json({ error: err.message || "Failed to import Shopify page" });
  }
});
