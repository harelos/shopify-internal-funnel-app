...
# Every prefix a purchased CJ order is filed under. Matching only RESCUE-{n}
# is how the Railway worker re-bought seventeen parcels on 2026-09-18 that the
# Cloudflare Worker had already ordered as AUTO-{n}.
_PURCHASE_RE = re.compile(r"^(?:RESCUE|AUTO|MANUAL|BACKFILL)-(\d+)$", re.IGNORECASE)
...
def supplier_orders_for(existing: dict[str, list[dict[str, Any]]], number: int) -> list[dict[str, Any]]:
    ...
    for order_num, copies in existing.items():
        match = _PURCHASE_RE.match(str(order_num or "").strip())
