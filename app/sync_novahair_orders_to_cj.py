...
Safety model is inherited unchanged from rescue_current_novahair_orders.py:
  ...
  * idempotent on any purchased CJ order for the sale (RESCUE-, AUTO-, MANUAL-, BACKFILL-)
...
def run(apply: bool, days: int, limit: int) -> int:
    token = get_token()
    ...
    for order in sorted(orders, key=lambda o: int(o.get("order_number") or 0)):
        ...
        duplicates = rescue.supplier_orders_for(existing, number)
        if len(duplicates) > 1:
