...
import { getShopifyConfig, isValidShopDomain, normalizeShopDomain } from "../lib/shopify-config.js";
...
export function verifyShopifySessionToken(token: string): ShopifySessionClaims {
  ...
  if (parts.length !== 3 || !config.clientId || !process.env.SHOPIFY_CLIENT_SECRET) {
...
export function requireShopifySession(req: Request, res: Response, next: NextFunction) {
  ...
  if ((req.path === "/track" || req.path === "/shopify/pixel") && (req.path === "/shopify/pixel" || verifyShopifyAppProxyRequest(req))) return next();
