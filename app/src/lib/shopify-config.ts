...
export function getShopifyConfig(): ShopifyRuntimeConfig {
  const shopDomain = normalizeShopDomain(process.env.SHOP_DOMAIN ?? process.env.ALLOWED_SHOP_DOMAIN);
  const distributionValue = clean(process.env.SHOPIFY_DISTRIBUTION).toLowerCase();
  ...
  return {
    shopDomain,
    appUrl: clean(process.env.APP_URL ?? process.env.APPLICATION_URL),
    clientId: clean(process.env.SHOPIFY_CLIENT_ID ?? process.env.SHOPIFY_API_KEY),
    apiVersion: clean(process.env.SHOPIFY_API_VERSION) || "2026-07",
    scopes: clean(process.env.SHOPIFY_SCOPES)
      .split(",")
      .map(scope => scope.trim())
      .filter(Boolean),
    liveConnect: process.env.SHOPIFY_LIVE_CONNECT === "true",
    requireEmbeddedAuth: process.env.SHOPIFY_REQUIRE_AUTH === "true",
    distribution,
    // Do not fall back to the old SHOPIFY_API_SECRET name. That variable was
    // used by the removed legacy route and must be rotated, not reused.
    hasClientSecret: Boolean(clean(process.env.SHOPIFY_CLIENT_SECRET)),
    hasAccessToken: Boolean(clean(process.env.SHOPIFY_ACCESS_TOKEN)),
  };
}
