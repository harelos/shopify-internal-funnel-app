...
import { detectSupportSentiment } from "./sentiment.js";
...
export function runSupportAgentSimulation(input: SupportAgentInput): SupportAgentResult {
  ...
  const sentiment = detectSupportSentiment(input.subject, input.message);
  ...
  if (input.facts?.order || input.facts?.customer) truthSources.unshift("SHOPIFY");
