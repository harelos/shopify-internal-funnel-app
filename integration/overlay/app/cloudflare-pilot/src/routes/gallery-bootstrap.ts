...
// Mounted after the existing requireShopifySession middleware.
...
galleryBootstrapAdmin.post('/element-experiments/:id/bootstrap-publish',async(req,res)=>{
  ...
  try {
    const result=await runBootstrapCommand(String(req.params.id),'sync',undefined,sessionToken);
    return res.status(result.bootstrap.state==='PUBLISHED'?200:409).json(result);
  }catch(error:any){return res.status(409).json({error:error.message});}
});
...
// Same app buttons, serialized publication handshake, opt-in experiment only.
for(const action of ['start','pause','promote'] as const) {
  ...
  galleryBootstrapAdmin.post(`/element-experiments/:id${suffix}`,async(req,res,next)=>{
    const id=String(req.params.id);if(!bootstrapConfiguredFor(id))return next();
    try {
      const token=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'')||undefined;
      const result=await runBootstrapCommand(id,action,req.params.variantId?String(req.params.variantId):undefined,token);
